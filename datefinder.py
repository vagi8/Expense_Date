        ...
        if not found_range:
            for match in DATE_REGEX.finditer(text):
                ...
                if strict:
                    ...
                    if len(digits)==3:    # 12-05-15
                        if (self.base_date.year-2000) in a and any(i <= self.base_date.month for i in a): #this year validation with month
                            complete = True
                        if (self.base_date.year-2001) in a: # last year validation
                            complete = True
                    elif (len(digits) == 2) and (len(years)==1):  # 12-05-2015
                        if self.base_date.year==int(years[0]) and any(i <= self.base_date.month for i in a): #this year validation with month
                            complete = True
                        if (self.base_date.year-1)==int(years[0]): # last year validation
                            complete = True
#                    elif (len(digits) == 2):  # 12-05
#                        complete = True
                    elif (len(months) == 1) and (len(digits) == 1): #27 Mar
                        complete = True
                        #conditions
                    elif (len(months) == 1) and (len(digits) == 2):  # 19 February 2013 year 09:10
                        complete = True
                    elif (len(months)==1) and (len(years)==1) and (len(digits)==1):
                        complete = True
                    if not complete:
                        continue
                    ...
                    while(it<len(a)):
                        if len(a[it])>4:
                            try:
                                ...
                                it+=1
                        if (len(a[it])==4):
                            if (int(a[it])==self.base_date.year) or (int(a[it])==(self.base_date.year-1)):
                                it+=1
                            else:
                                a.remove(a[it])
                        else:
...
#    return list( date_finder.find_dates(text, source=source, index=index, strict=strict))
    a=list( date_finder.find_dates(text, source=source, index=index, strict=strict))
#    it=0
#    while(it<len(a)):
#        i=a[it]
#        if i.year==base_date.year:
#            if i.month<=base_date.month and i.day<=base_date.day:
#                it+=1
#            else:
#                a.remove(i)
#        elif i.year<(base_date.year-1) or i.year>(base_date.year) :
#            a.remove(i)
#        else:
#            it+=1
    return a
